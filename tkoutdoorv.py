...
def top2(a,b):
    ...
    def update(section, key, value):
        ...
        with open('./OUTDOOR/gi_config_out.ini', 'w') as output:
            config.write(output)
    ...
    def autodoors():
        ...
        os.system('python ./OUTDOOR/auto_child_doors.py')
    ...
    def singledoors():
        ...
        os.system('python ./OUTDOOR/manual_child_doors.py')
    ...
    def automp():
        ...
        os.system('python ./OUTDOOR/auto_mp.py')

    def autochannel():
        update('doors','annotation',varanno.get())
        update('doors','text_box',vartext.get())

        os.system('python ./OUTDOOR/child_parts.py')
    ...
    def save():
        ...
        update('shell','z_channel_top_bottom_size',zt.get())
        update('shell','z_channel_side_size',zs.get())
    ...
    head = customtkinter.CTkLabel(a,text='OUTDOOR PANEL --- VERTICAL TYPE')
    ...
    labelz1= customtkinter.CTkLabel(frame1,text="Z channel Top&bottom size :")
    labelz1.grid(row=5,column=0,padx=10,pady=5)

    zt = customtkinter.CTkEntry(frame1,width=100)
    zt1 = config.get('shell','z_channel_top_bottom_size')
    zt.insert(0,zt1)
    zt.grid(row=5,column=1,padx=15,pady=5)

    labelz2= customtkinter.CTkLabel(frame1,text="Z channel Side size :")
    labelz2.grid(row=5,column=2,padx=20,pady=5)

    zs = customtkinter.CTkEntry(frame1,width=100)
    zs1 = config.get('shell','z_channel_side_size')
    zs.insert(0,zs1)
    zs.grid(row=5,column=3,padx=15,pady=5)
